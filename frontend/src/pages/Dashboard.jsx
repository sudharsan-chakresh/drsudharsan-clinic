...
import { Calendar, ListOrdered, AlertTriangle, Receipt, Clock, User, Stethoscope } from "lucide-react";
...
export default function Dashboard({ appointments, queue, stock, staff, invoices, user }) {
  ...
  const getGreeting = () => {
    ...
    return `${timeGreeting}, Dr. ${user.name.split(" ").pop()}`;
  };

  const getSubtitle = () => {
    switch (user?.role) {
      case "Admin":
        return "Here's the clinic overview and operations summary.";
      case "Doctor":
        return "Here's your schedule and patient queue for today.";
      case "Receptionist":
        return "Here's the front desk operations and appointments overview.";
      case "Pharmacist":
        return "Here's the stock overview and prescription queue.";
      case "Patient":
        return "Here's your upcoming appointments and consultation history.";
      default:
        return "Here's how the clinic is looking today.";
    }
  };

  return (
    <>
      <SectionHeader eyebrow="Overview" title={getGreeting()} subtitle={getSubtitle()} />
        ...
        <div className="two-col">
          <div className="stack">
            <Panel title="My Appointments">
              <table className="dtable">
                <thead>
                  <tr><th>Time</th><th>Doctor</th><th>Type</th><th>Status</th></tr>
                </thead>
                <tbody>
                  {appointments.slice(0, 5).map((a) => (
                    <tr key={a.id}>
                      <td style={{ color: "var(--muted)", fontWeight: 600 }}>{a.time}</td>
                      <td style={{ fontWeight: 700 }}>{a.doctor}</td>
                      <td>{a.type}</td>
          ...
          <div className="stack">
            <Panel title="My Consultations">
              <p style={{ fontSize: 13.5, color: "var(--muted)" }}>View your consultation history and prescriptions in the Consultations tab.</p>
            </Panel>
          </div>
        </div>
      ) : (
